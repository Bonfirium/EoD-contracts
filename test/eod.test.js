...
contract('EoD', (accounts) => {
	...
	describe('find_game()', () => {
		...
		it('successful', async () => {
			...
			await self.find_game.call({ from: addrs[7] }).then((res) => ok(res.eq(game_id.plus(1))));
		});
		//it('get_map(lobby_id)', () => {});	
	});
	describe('is_registred()', async () => {
		it('true', () => self.is_registred.call({ from: accounts[3] }).then((res) => strictEqual(res, true)));
		it('false', async () => {
			const addr = accounts[15]
			await self.is_registred.call({ from: addr }).then((res) => strictEqual(res, false));
		});
	});
});
